...
void main() {
    ...
    //fresnle effect
    float fresnleAmount = 5.0;
    float fresnleSpread=0.6;
    float fresnle = pow(1.2 - dot(vNormal, vec3(0.0, 0.0, 1.0)), fresnleAmount) * fresnleAmount * fresnleSpread;
    ...
    vec3 atmoShaded = flattenVec3(atmosphereMask * (fresnle + clouds) * atmoColor);
